properties {
	...
	$sln_file = "$src_directory\IdentityServer3.MembershipReboot.sln"
...
task RunTests -depends Compile {
	$project = "IdentityServer3.MembershipReboot.Tests"
...
task CreateNuGetPackage -depends Compile {
	...
	md $dist_directory
		...
		exec { . $nuget_path pack $src_directory\MembershipRebootUserService\MembershipRebootUserService.csproj -o $dist_directory -version $packageVersion }
}
